...
### keep curr_block_num as fixed effect and always keep (1|subj_id)
``` {r block_lmer_models}
complete_data <- na.omit(final_df_excluded[, c("correct", "rt","curr_block_num", "game_type", "preceding_bid_z_score","z_overall_correct", "preceding_bid", "block_switched", "subj_id", "sr_practice_accuracy", "vs_practice_accuracy", "avg_block_accuracy", "avg_block_rt", "trial_index")])

# do the models from before (other than the singular ones)
fit.compact_st_z = lmer(formula = correct ~ (1|subj_id),
                   data = complete_data)

fit.augmented_st_one_z = lmer(formula = correct ~ curr_block_num + (1|subj_id),
                     data = complete_data)

fit.augmented_st_two_z = lmer(formula = correct ~ preceding_bid + game_type + curr_block_num + (1|subj_id),
                     data = complete_data)

fit.augmented_st_three_z = lmer(formula = correct ~ preceding_bid + game_type + block_switched + curr_block_num + (1|subj_id),
                     data = complete_data)

fit.augmented_st_four_z = lmer(formula = correct ~ game_type + preceding_bid*block_switched + curr_block_num + (1|subj_id),data = complete_data)
...
# compare the models
compare_performance(fit.compact_st_z, fit.augmented_st_one_z, fit.augmented_st_two_z, fit.augmented_st_three_z, fit.augmented_st_four_z,rank = TRUE, verbose = FALSE)

summary(fit.augmented_st_four_z)
```
...
### keep curr_block_num as a fixed effect and always keeping (1|subj_id)
``` {r block_lmer_models}
complete_data <- na.omit(final_df_excluded[, c("curr_block_num", "game_type", "preceding_bid_z_score", "preceding_bid", "block_switched", "subj_id", "sr_practice_accuracy", "vs_practice_accuracy", "avg_block_accuracy", "avg_block_rt", "trial_index", "avg_z_accuracy", "following_bid_z_score")])%>%
  na.omit() %>%
  distinct() %>%
...
fit.compact_b_z = lmer(formula = avg_block_accuracy ~ (1 | subj_id),
                   data = complete_data)

fit.augmented_b_one_z = lmer(formula = avg_block_accuracy ~ curr_block_num + (1|subj_id),
                     data = complete_data)

fit.augmented_b_two_z = lmer(formula = avg_block_accuracy ~ curr_block_num + game_type + (1|subj_id),
                     data = complete_data)

fit.augmented_b_three_z = lmer(formula = avg_block_accuracy ~ preceding_bid + game_type + curr_block_num + (1|subj_id),
                     data = complete_data)

fit.augmented_b_four_z = lmer(formula = avg_block_accuracy ~ preceding_bid + game_type + block_switched + curr_block_num + (1|subj_id),data = complete_data) 

fit.augmented_b_five_z = lmer(formula = avg_block_accuracy ~ preceding_bid*block_switched + curr_block_num + game_type + (1|subj_id),data = complete_data)

fit.augmented_b_six_z = lmer(formula = avg_block_accuracy ~ preceding_bid*block_switched + game_type + curr_block_num + (1|subj_id),data = complete_data)

fit.augmented_b_seven_z = lmer(formula = avg_block_accuracy ~ game_type + preceding_bid*block_switched +curr_block_num+ (1+game_type|subj_id),data = complete_data) 

fit.augmented_b_eight_z = lmer(formula = avg_block_accuracy ~ preceding_bid*block_switched +curr_block_num+ (1+game_type|subj_id),data = complete_data) 

fit.augmented_b_nine_z = lmer(formula = avg_block_accuracy ~ preceding_bid*block_switched +curr_block_num + (1|subj_id),data = complete_data) 

# compare the models
compare_performance(fit.compact_b_z, fit.augmented_b_one_z, fit.augmented_b_two_z, fit.augmented_b_three_z, fit.augmented_b_four_z,  fit.augmented_b_five_z, fit.augmented_b_six_z, fit.augmented_b_seven_z, fit.augmented_b_eight_z,fit.augmented_b_nine_z,rank = TRUE, verbose = FALSE)

summary(fit.augmented_b_eight_z)

summary(fit.augmented_b_seven_z)
...
## add curr block num as fixed, and always (1|subj_id)

``` {r block_lmer_models}

complete_data <- (final_df_excluded[, c("curr_block_num", "correct", "game_type", "preceding_bid", "block_switched", "subj_id", "sr_practice_accuracy", "vs_practice_accuracy", "avg_block_accuracy", "avg_block_rt", "following_bid_z_score","following_bid", "z_overall_correct", "avg_z_accuracy")]) %>%
  na.omit() %>%
  arrange(subj_id, curr_block_num)

# for bid predictions
fit.compact_bid = lmer(formula = following_bid ~ (1 | subj_id),
                   data = complete_data)

fit.augmented_bid_one = lmer(formula = following_bid ~ curr_block_num + (1|subj_id),
                     data = complete_data)

fit.augmented_bid_two = lmer(formula = following_bid ~ game_type + curr_block_num + (1|subj_id),
                     data = complete_data)

fit.augmented_bid_three = lmer(formula = following_bid ~ curr_block_num + (1 + game_type|subj_id),
                     data = complete_data)

fit.augmented_bid_four = lmer(formula = following_bid ~ curr_block_num + game_type + (1 + game_type|subj_id),
                     data = complete_data)

fit.augmented_bid_five = lmer(formula = following_bid ~ correct + curr_block_num + game_type + (1 + game_type|subj_id),
                     data = complete_data)

fit.augmented_bid_six = lmer(formula = following_bid ~ correct + curr_block_num + block_switched + (1 + game_type|subj_id),
                     data = complete_data)

fit.augmented_bid_seven = lmer(formula = following_bid ~ avg_block_accuracy + curr_block_num + block_switched + (1 + game_type|subj_id),
                     data = complete_data)

compare_performance(fit.compact_bid, fit.augmented_bid_one, fit.augmented_bid_two, fit.augmented_bid_three, fit.augmented_bid_four, fit.augmented_bid_five,fit.augmented_bid_six, fit.augmented_bid_seven, rank = TRUE, verbose = FALSE)

summary(fit.augmented_bid_seven)

```


## add curr block num as fixed, and always (1|subj_id); focus on using avg block acc

``` {r block_lmer_models}

complete_data <- (final_df_excluded[, c("curr_block_num", "game_type", "preceding_bid", "block_switched", "subj_id", "sr_practice_accuracy", "vs_practice_accuracy", "avg_block_accuracy", "avg_block_rt", "following_bid_z_score","following_bid", "z_overall_correct", "avg_z_accuracy")]) %>%
  na.omit() %>%
  distinct() %>%
  arrange(subj_id, curr_block_num)
...
fit.augmented_bid_five = lmer(formula = following_bid ~ avg_block_accuracy + curr_block_num + game_type + (1 + game_type|subj_id),
                     data = complete_data)

fit.augmented_bid_six = lmer(formula = following_bid ~ avg_block_accuracy + curr_block_num + block_switched + (1 + game_type|subj_id),
                     data = complete_data)

fit.augmented_bid_seven = lmer(formula = following_bid ~ avg_block_accuracy + curr_block_num  + (1 + game_type|subj_id) + (1|block_switched),
                     data = complete_data)

fit.augmented_bid_eight = lmer(formula = following_bid ~ avg_block_accuracy + curr_block_num  + block_switched + game_type + (1 + game_type|subj_id), data = complete_data)


fit.augmented_bid_nine = lmer(formula = following_bid ~ avg_block_accuracy + curr_block_num  + block_switched + (1+game_type|subj_id), data = complete_data)
summary(fit.augmented_bid_nine)

fit.augmented_bid_ten = lmer(formula = following_bid ~ avg_block_accuracy + curr_block_num*block_switched + (1+game_type|subj_id), data = complete_data)
summary(fit.augmented_bid_ten)

compare_performance(fit.compact_bid, fit.augmented_bid_one, fit.augmented_bid_two, fit.augmented_bid_three, fit.augmented_bid_four, fit.augmented_bid_five,fit.augmented_bid_six, fit.augmented_bid_seven, fit.augmented_bid_eight, fit.augmented_bid_nine, fit.augmented_bid_ten,rank = TRUE, verbose = FALSE)

summary(fit.augmented_bid_six)
summary(fit.augmented_bid_eight)
```