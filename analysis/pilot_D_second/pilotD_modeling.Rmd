...
# 
# # calculate average (and z) accuracy and average (and z) rt per block. (this is when it is z'd only by subject ACROSS tasks)
# avg_per_block <- final_df %>%
#   group_by(subj_id, curr_block_num) %>%
#   summarise(avg_z_overall_accuracy = mean(z_overall_correct),stdev_z_overall_accuracy = sd(z_overall_correct,na.rm = TRUE), avg_z_overall_rt= mean(z_overall_rt),stdev_z_overall_rt = sd(z_overall_rt,na.rm = TRUE))%>%
#   arrange(subj_id,curr_block_num)
# 
# final_df <- final_df %>%
#   left_join(avg_per_block, by=c("subj_id", "curr_block_num"))
...
## (normalized per participant PER task) Block Accuracy and RT by (normalized per participant) Following Bids 
...
## Relationship between (normalized per participant) bid offer, switch status and following (normalized per participant PER task) average accuracy and RT
...
```
# SAME GRAPHS BUT NORMALIZED BY PARTICIPANT PERFORMANCE OVERALL (NOT PER TASK)

This way, we can see how they perform in each task relative to their typical performance, in order to adjust for participants but not account for which task they're doing -- could be skewed because some participants perform a lot worse on one task than another so the average will be lower (but normalization should account for that?)


## (normalized per participant ACROSS tasks) Block Accuracy and RT by (normalized per participant) Following Bids 

Normalizing per participant means we are looking at **how well participants perform relative to how well they normally perform in the experiment overall**
...
# including all subjects
final_df_excluded %>%
  group_by(subj_id, following_bid_z_score, avg_z_overall_block_accuracy) %>%
  ggplot(mapping = aes(x = following_bid_z_score, y = avg_z_overall_block_accuracy)) +
  geom_smooth(method="lm",se=TRUE) +
  geom_point(mapping = aes(color = curr_block_num), size = 2) +  # Points colored by block number
  labs(x = "Bid Z-Score",
       y = "Average Accuracy in Previous Block, z-scored by subject only",
       color = "Block Number") +
  ggtitle("Prior Block's Average z-scored Accuracy by Z-Scored Bid, all subjects")
...
## Relationship between (normalized per participant) bid offer, switch status and following (normalized per participant ACROSS tasks) average accuracy and RT

Normalizing per participant means we are looking at **how well participants perform relative to how well they normally perform overall**
...
temp <- final_df_excluded %>%
  group_by(subj_id, curr_block_num, avg_z_overall_block_accuracy, following_bid_z_score, preceding_bid_z_score, block_switched) %>%
  summarise(preceding_bid_z_score = mean(preceding_bid_z_score), following_bid_z_score =mean(following_bid_z_score), avg_z_accuracy = mean(avg_z_overall_block_accuracy)) %>%
  mutate(bid_bin_preceding = cut(preceding_bid_z_score, breaks = breaks, labels = labels, right = FALSE),
         bid_bin_following = cut(following_bid_z_score, breaks = breaks, labels = labels, right = FALSE))%>%
  mutate(block_switched = ifelse(block_switched == 'false',"no switch","switch"))
...
temp %>%
  ...
  stat_summary(fun.data = "mean_cl_boot",
               ...
               shape = 21,
               size = 0.5)+
  labs(x="Did the task switch?",y="Average (z-scored per participant) accuracy in this block")+
  ggtitle("bid offer (z-scored), switch status and following average accuracy (z-scored/participant)")
```
...
fit.augmented_st_eight = lmer(formula = correct ~ preceding_bid*block_switched + (1 + preceding_bid|subj_id) + (1|curr_block_num) + (game_type|subj_id),data = complete_data)

fit.augmented_st_nine = lmer(formula = correct ~ preceding_bid*block_switched + (1 + preceding_bid|subj_id) + (1|curr_block_num) + (1|game_type),data = complete_data)
```
...
fit.augmented_b_four = lmer(formula = avg_block_accuracy ~ preceding_bid_z_score + game_type + block_switched + (1 + curr_block_num|subj_id),data = complete_data)
...
fit.augmented_b_seven = lmer(formula = avg_block_accuracy ~ game_type + preceding_bid_z_score*block_switched + (1 + game_type|subj_id) + (1|curr_block_num),data = complete_data)
...
complete_data <- (final_df_excluded[, c("curr_block_num", "correct", "game_type", "preceding_bid", "block_switched", "subj_id", "sr_practice_accuracy", "vs_practice_accuracy", "avg_block_accuracy", "avg_block_rt", "following_bid_z_score")]) %>%
  na.omit() %>%
  arrange(subj_id, curr_block_num)
...
fit.augmented_bid_one = lmer(formula = following_bid_z_score ~ (1 + curr_block_num|subj_id),
                     data = complete_data)

fit.augmented_bid_two = lmer(formula = following_bid_z_score ~ game_type + (1|subj_id) + (1|curr_block_num),
                     data = complete_data)

fit.augmented_bid_three = lmer(formula = following_bid_z_score ~ game_type + (1 + game_type|subj_id) + (1|curr_block_num),
                     data = complete_data)

fit.augmented_bid_four = lmer(formula = following_bid_z_score ~ correct + game_type + (1 + game_type|subj_id) + (1|curr_block_num),
                     data = complete_data)

fit.augmented_bid_five = lmer(formula = following_bid_z_score ~ correct + game_type + (1 + game_type|subj_id) + (1|curr_block_num) + (1|block_switched),
                     data = complete_data)

fit.augmented_bid_six = lmer(formula = following_bid_z_score ~ correct + game_type + (1 + game_type + curr_block_num|subj_id) + (1|block_switched),
                     data = complete_data)

# compare them
anova(fit.compact_bid, fit.augmented_bid_one) # one
anova(fit.augmented_bid_one, fit.augmented_bid_two) # unclear
anova(fit.augmented_bid_one, fit.augmented_bid_three) # three
anova(fit.augmented_bid_two, fit.augmented_bid_three) # three is better
anova(fit.augmented_bid_three, fit.augmented_bid_four) # four is better
anova(fit.augmented_bid_four, fit.augmented_bid_five) # five
anova(fit.augmented_bid_five, fit.augmented_bid_six) # six

summary(fit.augmented_bid_six)
```

Being more correct negatively predicts a bigger bid significantly