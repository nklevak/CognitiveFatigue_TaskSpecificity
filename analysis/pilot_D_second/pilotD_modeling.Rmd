...
betas <- final_df_excluded %>%
  filter(trial_count == 1, curr_block_num != 1) %>% # to make it one data point per participant per block
  group_by(curr_block_num) %>%
  do(tidy(lm(avg_z_accuracy ~ preceding_bid_z_score, data = .))) %>%
  filter(term == "preceding_bid_z_score") %>%
...
ggplot(betas, aes(x = curr_block_num, y = estimate)) +
  geom_line() +
  geom_point() +
  labs(title = "Betas of z-preceding bid as a function of Block Number, to predict block z-acc",
       x = "Current Block Number",
       y = "Beta Coefficient for preceding_bid_z_score")
...
betas_2 <- final_df_excluded %>%
  ...
  group_by(curr_block_num) %>%
  do(tidy(lm(correct ~ preceding_bid_z_score*block_switched + (1|subj_id), data = .))) %>%
  filter(term == "block_switched") %>%
  select(curr_block_num, estimate)
