...
analysis_data_2 <- final_df_excluded %>%
  # Remove any NA values that might cause issues
  drop_na(following_bid, avg_block_accuracy, curr_block_num, subj_id) %>%
  select(following_bid, avg_block_accuracy, curr_block_num, subj_id, game_type) %>%
  distinct()
# 
# ### plotting the betas
# doing 1|subj_id here doesn't make sense
# # Calculate block-specific betas
# block_betas <- analysis_data_2 %>%
#   group_by(curr_block_num) %>%
#   summarise(
#     model = list(lm(following_bid ~ avg_block_accuracy + (1|subj_id))),
#     tidy_model = list(tidy(model[[1]])),
#     r2 = summary(model[[1]])$r.squared
#   ) %>%
#   unnest(tidy_model) %>%
...
#   # Labels
#   labs(x = "Block Number",
#        y = "Beta Coefficient (Bid ~ Accuracy + (1| subject ID))",
#        title = "How Performance Predicts Subsequent Bids Across Blocks",
#        subtitle = "Red line shows linear trend across blocks",
#        color = "Statistical\nSignificance") +
#   theme_minimal() +
...
# 
# 
# # Option 2: Raw data with smooth lines
# ggplot(analysis_data, aes(x = preceding_bid_z_score, y = avg_block_accuracy,
...
#        y = "Accuracy",
#        color = "Switched Tasks",
#        title = "Relationship Between Previous Bid and Accuracy",
#        subtitle = "Separate trends for switch vs. stay trials") +
#   theme_minimal() +
#   scale_color_manual(values = c("blue", "red"),
#                     labels = c("Stayed", "Switched"))


# Option 2: Coefficient plot
