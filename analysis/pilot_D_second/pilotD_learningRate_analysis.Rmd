...
c
# Automaticity/learning rate checks

Looking to see how improvement across trials happens over time / blocks of doing the experiment

``` {r automaticity}
automaticity <- final_df_excluded %>%
  select(subj_id, game_type, curr_block_num, task_block_count, trial_count, correct, avg_block_accuracy, avg_z_accuracy, preceding_bid_z_score, following_bid_z_score, block_switched)

# add a column for how many consecutive of this task there were
# per task, per task block count, plot correct over time for all trials in the block

automaticity_con <- automaticity %>%
  arrange(subj_id, curr_block_num, trial_count) %>%
  group_by(subj_id, game_type) %>%
  mutate(trial_count_con = ifelse(game_type == "vs", trial_count + (task_block_count-1)*vs_per_block, trial_count + (task_block_count-1)*sr_per_block)) %>%
  ungroup()

list_block_switch <- automaticity_con %>%
  filter(block_switched=="true") %>%
  group_by(subj_id, curr_block_num) %>%
  mutate(first_block_switch = min(trial_count_con)) %>%
  distinct(subj_id, game_type, curr_block_num, first_block_switch) %>%
  ungroup()

automaticity_con <- automaticity_con %>%
  arrange(subj_id, curr_block_num, trial_count_con)

# Step 2: Create the switch group
automaticity_con <- automaticity_con %>%
  group_by(subj_id, game_type) %>%
  mutate(switch_group = cumsum(block_switched == "true" & trial_count == 1)) %>%
  ungroup()

automaticity_con <- automaticity_con %>%
  mutate(switch_group = ifelse(game_type == "vs", switch_group, switch_group + 30))


# Plot
#sr
# Sample participants
sampled_participants <- sample(unique(automaticity_con$subj_id), 4)  # Adjust sample size as needed

# Plot with GLM for each segment between switches
automaticity_con %>%
  filter(game_type == "spatial_recall", subj_id %in% sampled_participants) %>%
  ggplot(mapping = aes(x = trial_count_con, y = correct, group = switch_group, color = curr_block_num)) +
  geom_point(alpha = 0.3, position = position_jitter(0.2, 0.1)) +
  
  # Fit GLM for each segment (defined by switch_group)
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE) +
  
  # Add vertical lines where the block switched for each participant
  geom_vline(data = list_block_switch %>% filter(game_type == "spatial_recall", subj_id %in% sampled_participants),
             aes(xintercept = first_block_switch), linetype = "dashed", color = "red", size = 0.1) +
...
# Plot with GLM for each segment between switches
automaticity_con %>%
  filter(game_type == "vs", subj_id %in% sampled_participants) %>%
  ggplot(mapping = aes(x = trial_count_con, y = correct, group = switch_group, color = curr_block_num)) +
  geom_point(alpha = 0.3, position = position_jitter(0.2, 0.1)) +
  ...
  # Add vertical lines where the block switched for each participant
  geom_vline(data = list_block_switch %>% filter(game_type == "vs", subj_id %in% sampled_participants),
             aes(xintercept = first_block_switch), linetype = "dashed", color = "red", size = 0.1) +
  
  # Facet by subject ID
  facet_wrap(~subj_id, scales = "free_y") +
  theme_minimal()


# all together 
sampled_participant <- sample(unique(automaticity_con$subj_id), 1)  # Adjust sample size as needed
# Plot with GLM for each segment between switches
automaticity_con %>%
  filter(subj_id %in% sampled_participant) %>%
  ggplot(mapping = aes(x = trial_count_con, y = correct, group = interaction(game_type,switch_group), color = game_type)) +
  geom_point(alpha = 0.3, position = position_jitter(0.2, 0.1)) +
  ...
  # Add vertical lines where the block switched for each participant
  geom_vline(data = list_block_switch %>% filter(subj_id %in% sampled_participant, game_type=="vs"),
             aes(xintercept = first_block_switch), linetype = "dashed", color = "blue", size = 0.1) +
  geom_vline(data = list_block_switch %>% filter(subj_id %in% sampled_participant, game_type=="spatial_recall"),
             aes(xintercept = first_block_switch), linetype = "dashed", color = "red", size = 0.1) +
  # Facet by subject ID
  theme_minimal()
```
...
Trying to plot this all on same axis
``` {r automaticity}
automaticity <- final_df_excluded %>%
  select(subj_id, game_type, curr_block_num, task_block_count, trial_count, correct, avg_block_accuracy, avg_z_accuracy, preceding_bid_z_score, following_bid_z_score, block_switched)
...
automaticity_con <- automaticity %>%
  ...
  group_by(subj_id) %>%
  arrange(subj_id, curr_block_num, trial_count_con) %>%
  mutate(trial_count_con_total = ifelse(game_type == "vs", trial_count_con + (curr_block_num - task_block_count)*sr_per_block, trial_count_con + (curr_block_num - task_block_count)*vs_per_block)) %>%
  ungroup()

list_block_switch <- automaticity_con %>%
  filter(block_switched=="true") %>%
  group_by(subj_id, curr_block_num) %>%
  mutate(first_block_switch = min(trial_count_con), first_block_switch_total = min(trial_count_con_total)) %>%
  distinct(subj_id, game_type, curr_block_num, first_block_switch, first_block_switch_total) %>%
  ungroup()
...
# Sample participants
# all together 
sampled_participant <- sample(unique(automaticity_con$subj_id), 4)  # Adjust sample size as needed
# Plot with GLM for each segment between switches
automaticity_con %>%
  filter(subj_id %in% sampled_participant) %>%
  ggplot(mapping = aes(x = trial_count_con_total, y = correct, group = interaction(game_type,switch_group), color = game_type)) +
  geom_point(alpha = 0.1, position = position_jitter(0.1, 0)) +
  ...
  # Add vertical lines where the block switched for each participant
  geom_vline(data = list_block_switch %>% filter(subj_id %in% sampled_participant, game_type=="vs"),
             aes(xintercept = first_block_switch_total), linetype = "dashed", color = "blue", size = 0.1) +
  geom_vline(data = list_block_switch %>% filter(subj_id %in% sampled_participant, game_type=="spatial_recall"),
             aes(xintercept = first_block_switch_total), linetype = "dashed", color = "red", size = 0.1) +
  # Facet by subject ID
  facet_wrap(~subj_id, nrow=4) +
  theme_minimal()
```

Now plot this across participants:
``` {r automaticity}

library(broom)  # For tidy extraction of coefficients

# Check if any groups have only 0s or 1s
separation_check <- automaticity_con %>%
  group_by(subj_id, switch_group) %>%
  summarize(all_zeros = all(correct == 0), all_ones = all(correct == 1)) %>%
  filter(all_zeros | all_ones)

# View problematic groups
separation_check

# Filter out groups with all 0s or all 1s
filtered_data <- automaticity_con %>%
  group_by(subj_id, game_type, switch_group) %>%
  filter(!all(correct == 0) & !all(correct == 1))


filtered_data <- automaticity_con %>%
  group_by(subj_id, game_type, switch_group) %>%
  filter(sum(correct == 0) > 0 & sum(correct == 1) > 0) %>%
  filter(n() > 5)  # Ensures groups with too few data points are also removed

library(purrr)

# Safely fit GLMs to avoid errors
safe_glm <- safely(function(data) {
  glm(correct ~ trial_count_con_total, data = data, family = binomial)
})

# Fit GLMs for each participant and switch group with error handling
glm_models <- filtered_data %>%
  group_by(subj_id, game_type, switch_group) %>%
  nest() %>%
  mutate(model = map(data, safe_glm))  # Apply safely wrapped glm function

# Extract successful models
glm_models <- glm_models %>%
  mutate(success = map_lgl(model, ~ is.null(.x$error)),
         result = map(model, "result"))

# Filter out failed models
successful_models <- glm_models %>%
  filter(success)

# View the results of successful models
successful_models
library(broom)

# Extract coefficients from successful models
coefficients <- successful_models %>%
  mutate(coefs = map(result, tidy)) %>%
  unnest(coefs) %>%
  filter(term == "(Intercept)")  # Only keep intercepts

# Plot the intercepts across participants and switch groups
ggplot(coefficients %>% filter(subj_id <= 10), aes(x = switch_group, y = estimate, color = subj_id)) +
  geom_point(size = 2) +
  geom_point(aes(group = subj_id)) +
  facet_grid(subj_id ~ game_type) +  # Facet by subj_id and game_type
  labs(x = "Switch Group", y = "Intercept (log-odds)", color = "Participant") +
  theme_minimal() +
...
coefficients <- successful_models %>%
  mutate(coefs = map(result, tidy)) %>%
  unnest(coefs) %>%
  filter(term == "trial_count_con_total")  # Only keep learning rate
...
ggplot(coefficients %>% filter(subj_id <= 10), aes(x = switch_group, y = estimate, color = subj_id)) +
  ...
  geom_point(aes(group = subj_id)) +
  facet_grid(subj_id ~ game_type) +  # Facet by subj_id and game_type
  labs(x = "Switch Group", y = "learning rate over time", color = "Participant") +
  theme_minimal() +
  theme(legend.position = "bottom")



# make a graph across all subjects
...
# Plot the intercepts across participants and switch groups
ggplot(coefficients, aes(x = switch_group, y = estimate, color = subj_id)) +
  geom_point(size = 2) +
  geom_point(aes(group = subj_id)) +
  facet_wrap(~game_type) +  # Facet by subj_id and game_type
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "Switch Group", y = "learning rate over time", color = "Participant") +
  theme_minimal() +
  theme(legend.position = "bottom")
...
coefficients <- successful_models %>%
  mutate(coefs = map(result, tidy)) %>%
  unnest(coefs) %>%
  filter(term == "(Intercept)")  # Only keep learning rate
...
ggplot(coefficients, aes(x = switch_group, y = estimate, color = subj_id)) +
  ...
  facet_wrap(~game_type) +  # Facet by subj_id and game_type
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "Switch Group", y = "intercept over time", color = "Participant") +
  theme_minimal() +
  theme(legend.position = "bottom")
