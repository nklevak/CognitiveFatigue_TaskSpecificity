...
date: "2024-07-03"
---
This is the updated analysis on the pilot C participants (the SONA ones). In this analysis, I z-scored the participant performance within each task in order to better compare across the two tasks. 
The spatial recall was 4 squares at a time, 
...
Here, we will exclude participants. We want to exclude participants who never switched. We also want to exclude participants who got over **98% accuracy** on **both** tasks. 
...
bid_block_total %>%
  ...
  labs(x = "Following Bid Z-Score",
       ...
       color = "Block Number") +
  ggtitle("Average Z-scored Accuracy by Following Bid Z-Score for Each Block, across subjects")
...
bid_block_total %>%
  ...
  labs(x = "Following Bid Z-Score",
       y = "Average Accuracy",
       color = "Block Number") +
  ggtitle("Average Z-scored Accuracy by Following Bid Z-Score for Each Block, by subject")
...
excluded_switch_data %>%
  mutate(subj_id = as.factor(subj_id)) %>%
  mutate(block_switched = ifelse(is.na(block_switched),"block did not switch", ifelse(block_switched == 0,"block did not switch","block switched"))) %>%
  group_by(subj_id) %>%
