...
# already_paid_2 <- read.csv("SPR_main_approvals_batch2.txt",header = FALSE,           
#                         col.names = "prolific_id",# Names the single column
#                         stringsAsFactors = FALSE)
```
...
df_recent_subset <- df_all %>%
  ...
  mutate(is_correct_numeric = ifelse(is_correct=="false" | timed_out == "1" | timed_out == "true", 0, 1)) %>%
...
df_bonus_id %>%
  ...
  # filter(!(prolific_id %in% already_paid_1$prolific_id)) %>% #this lets me filter out those I already approved/looked at most recently (will need to figure out better system later)
  # filter(!(prolific_id %in% already_paid_2$prolific_id)) %>% #this lets me filter out those I already approved/looked at
  print() %>%
  select(subj_id, prolific_id, flagged_time_outs, max_timed_out_sr_ds, flagged_few_trials) %>%
  distinct()
# 
# df_bonus_id %>%
#   filter(prolific_id == "") %>%
...
prop_timed_out <- df_bonus_id %>%
  filter(!(subj_id %in% c(35))) %>% # exclude for now
...
# for batch 1
subj_to_exclude = c(35) #TODO here can add specific subj_id from people who timed out too much / did not participate (12,17,31 are fine)
...
1. accuracy below 30% on both tasks (digit span and spatial recall) or above 95% on both tasks
2. if they said they didn't read the cues (????? not sure about this one)
3. if they never used more than 1 rest (???? not sure)
