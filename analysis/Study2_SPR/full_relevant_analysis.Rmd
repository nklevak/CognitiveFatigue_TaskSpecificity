...
# POST-HOC POWER ANALYSIS FOR FUTURE REPLICATION
``` {r}
library(simr)
```

``` {r}
model_data_seq <- model_data_q2
model_data_seq$seq_id <- as.numeric(factor(model_data_q2$subj_id))

set.seed(999)

# Fit the pilot model
# simplified by removing game_type
# gmq2 <- glmer(cbind(next_block_successes, next_block_failures) ~ 
#                 avg_block_accuracy*cue_type + game_type + group_num + gameA_isSR + (1|seq_id),
#               family = binomial,
#               data = model_data_seq)

# SUPER SIMPLE MODEL:
gmq2 <- glmer(cbind(next_block_successes, next_block_failures) ~ avg_block_accuracy*cue_type + num_rest_in_chunk + (1|seq_id),
               family = binomial,
               data = model_data_seq)

# effect size and create the model for power analysis
effect_to_detect <- 0.1
model_for_power <- gmq2
fixef(model_for_power)["avg_block_accuracy:cue_typeswitch"] <- effect_to_detect
...
for (i in 1:length(breaks)) {
  ...
  n_current <- breaks[i]
  cat(paste0("(", i, "/", length(breaks), ") Running for N = ", n_current, "\n")) # Progress indicator
  
  # Extend the model to the current sample size
  temp_model <- extend(model_for_power, along = "seq_id", n = n_current)
  
  # Run the power simulation for this specific size
  sim_result <- powerSim(temp_model, 
                         test = fixed("avg_block_accuracy:cue_typeswitch", "z"), 
                         nsim = nsim)
  
  # Store the result
  power_results[[i]] <- sim_result
}

power_df <- data.frame(
  n = breaks,
  power = numeric(length(breaks)),
  lower = numeric(length(breaks)),
  upper = numeric(length(breaks))
)

for (i in 1:length(power_results)) {
  
  res <- power_results[[i]]
  
  # Number of significant results (from the p-values)
  num_significant <- as.integer(sum(res$pval < res$alpha, na.rm = TRUE))
  
  # Total number of attempts (the `nsim` you requested)
  total_attempts <- res$n
  
  # Calculate power: significant results / total attempts
  if (total_attempts > 0) {
    power_df$power[i] <- num_significant / total_attempts
  } else {
    power_df$power[i] <- NA
  }
  
  # Calculate the confidence interval using the same logic
  if (total_attempts > 0) {
    ci <- binom.test(num_significant, total_attempts)$conf.int
    power_df$lower[i] <- ci[1]
    power_df$upper[i] <- ci[2]
  } else {
    power_df$lower[i] <- NA
    power_df$upper[i] <- NA
  }
}
print("Final, Corrected Data Frame:")
print(power_df)

ggplot(power_df, aes(x = n, y = power)) +
  geom_point(size = 3) +
  geom_line() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 10, linewidth = 0.8) +
  geom_hline(yintercept = 0.8, linetype = "dashed", color = "red") +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(
    x = "Number of participants",
    y = "Power",
    title = "Power Curve with Convergence Failures",
    subtitle = "Q2: performance ~ avg_block_accuracy:cue_typeswitch"
  ) +
...
``` {r}
# do power analysis for Q1

# Best Model before: same as before but also spatial recall is a sig. neg predictor (if it was spatial recall you rest less)
# fit.mixed_5_nb <- glmer.nb(num_rest_in_chunk ~ group_num + avg_block_accuracy*cue_type + gameA_isSR + game_type + (1|subj_id),
#                           data = model_data)
model_data_seq <- model_data
model_data_seq$seq_id <- as.numeric(factor(model_data$subj_id))

# Verify the new ID
range(model_data_seq$seq_id)
length(unique(model_data_seq$seq_id))

set.seed(999)

gmq1 <- glmer.nb(num_rest_in_chunk ~ group_num + avg_block_accuracy*cue_type + gameA_isSR + game_type + (1|seq_id),
                 data = model_data_seq)
model_for_power <- gmq1

# sample sizes
breaks <- seq(50, 300, 50)
nsim <- 1000 # Use 100 for testing, 1000 for final results
...
for (i in 1:length(breaks)) {
  ...
  # Run the power simulation for this specific size
  sim_result <- powerSim(temp_model, 
                         test = fixed("avg_block_accuracy", "z"), 
                         nsim = nsim)
...
# 2. Loop through each result and fill the data frame
for (i in 1:length(power_results)) {
...
ggplot(power_df, aes(x = n, y = power)) +
  ...
  labs(
    x = "Number of participants",
    y = "Power",
    title = "Power Curve by Sample Size",
    subtitle = "Q1: rest amount ~ avg_block_accuracy"
  ) +
  theme_minimal(base_size = 14)
```
...
gmq3 <- glmer.nb(num_rest_in_chunk ~ group_num + avg_block_accuracy*cue_type + gameA_isSR + game_type + (1|seq_id),
                 data = model_data_seq)
model_for_power <- gmq3
...
# Loop through each sample size and get the power at that size (since powerCurve isnt working for some reason)
for (i in 1:length(breaks)) {
...
# 6. Combine and plot the results
power_df <- data.frame(
  n = breaks,
  power = numeric(length(breaks)),
  lower = numeric(length(breaks)),
...
ggplot(power_df, aes(x = n, y = power)) +
  ...
  labs(
    ...
    y = "Power",
    title = "Power Curve by Sample Size",
    subtitle = "Q3: rest amount ~ avg_block_accuracy:cue_typeswitch"
  ) +
  theme_minimal(base_size = 14)
```
# POST-HOC RT ANALYSIS
Deciding what exclusion criteria to use for the next study regarding reaction time for spatial recall or digit span
``` {r post_hoc_rt}
glimpse(df_all)

rt_post_hoc <- df_all %>%
  select(-gameA_name, -gameA_isSR, -sequence, -trial_type, -follows_group_num, -follows_internal_block_num, -type_desc)
rt_post_hoc

# some of the digit spans slightly longer than 4200 ms got through
# rt_post_hoc %>%
...
rt_post_hoc %>%
  filter(timed_out == FALSE, game_type != "rest_task") %>%
  ggplot(mapping = aes(y = rt, color = subj_id, group=subj_id)) +
  geom_boxplot(outlier.colour = "black", outlier.shape = 16,
               outlier.size = 0.5) +
  facet_wrap(~ game_type) +
  labs(title="distribution of rt per subject per game type (excluding time outs)")

rt_post_hoc %>%
  filter(timed_out == FALSE, game_type != "rest_task") %>%
  ggplot(mapping = aes(x=game_type, y = rt, color = game_type)) +
  geom_boxplot(outlier.colour = "black", outlier.shape = 16,
               outlier.size = 0.5) +
  labs(title="distribution of rt across all subjects per game type (excluding time outs)")

#######################################################
# looking at summary stats to decide on exclusion critera for next rep
# Calculate summary statistics for correct trials
rt_summary_correct <- rt_post_hoc %>%
  filter(timed_out == FALSE, game_type != "rest_task", is_correct_numeric == 1) %>%
...
print(rt_summary_correct)

rt_summary_correct %>%
  ungroup() %>%
  ggplot(mapping = aes(x=game_type, y=median_rt, group=game_type, color=game_type)) + 
  geom_boxplot(outlier.colour = "black", outlier.shape = 16,
               outlier.size = 0.5) + 
  geom_point(position = position_jitter(0.1,0), alpha = 0.2) +
  ylim(0, max(rt_summary$median_rt, na.rm = TRUE)) +
  labs(title="distribution of median reaction time of each subject for correct trials, per game type (excluding time outs)")


# DECISION: base future exclusion criteria per game type on 1) median RT of a subject being below the 5th percentile of median RT in this sample and 2) the sd of reaction time being below the 5th percentile of RT sd in this sample
# Calculate RT thresholds by game type
rt_thresholds <- rt_summary_correct %>%
  group_by(game_type) %>%
  summarise(
    p5_median_rt = quantile(median_rt, 0.05, na.rm = TRUE),
    p5_sd_rt = quantile(sd_rt, 0.05, na.rm = TRUE),
    .groups = 'drop'
  )
print(rt_thresholds)

# see how it would affect this sample: 
suspicious_subjects <- rt_summary_correct %>%
  mutate(
    too_fast = median_rt < quantile(median_rt, 0.05, na.rm = TRUE),
    too_consistent = sd_rt < quantile(sd_rt, 0.05, na.rm = TRUE),
  ) %>%
  group_by(subj_id) %>%
  summarise(
    n_too_fast = sum(too_fast, na.rm = TRUE),
    n_too_consistent = sum(too_consistent, na.rm = TRUE)
  ) %>%
  filter(n_too_fast > 0 & n_too_consistent > 0)
suspicious_subjects
