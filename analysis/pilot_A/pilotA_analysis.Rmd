...
full_with_bid <- full_with_bid %>%
  ...
  mutate(bid_value = if(any(fill_flag)) first(bid_value[fill_flag]) else bid_value, blocks_completed = if (any(fill_flag)) first(blocks_completed[fill_flag]) else blocks_completed) %>%
  ungroup() %>%
...
full_with_bid <- full_with_bid %>%
  ...
  select(-blocks_completed, -sequence, -backwards, -responses, -score_pc, -score_ls)
```
