# THIS CODE: runs a nonheirarchical HMM fit to each subject individually
import argparse
import json
import numpy as np
import pymc as pm
...
def fit_subject(subject_id, subject_data, n_samples=100, n_tune=500, n_states = 2, outdir="HMM_modeling/results"):
    ...
    with pm.Model() as model:
        ...
        class HMMCustomDist(pm.CustomDist):
            @staticmethod
            def dist(game, time, base_mu, beta_game, beta_time, sigma, pi, A, n_states, size=None):
                return pt.zeros((game.shape[0], 2))
                
            @staticmethod
            def logp(value, game, time, base_mu, beta_game, beta_time, sigma, pi, A, n_states):
        ...
        trace = pm.sample(n_samples, tune=n_tune,init='adapt_diag', target_accept=0.95, progressbar=False)
        # Save trace
...
if __name__ == "__main__":
    ...
    fit_subject(args.subject_id,
                subject_data,
                n_samples=2000,
                n_tune=2000,
                n_states=2,
                outdir="HMM_modeling/results_2state_n5000_t5000")
